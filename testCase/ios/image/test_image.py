...
class Test_image:
    ...
    def test_image(self):
        ...
        # 2.local image
        self.assertionUtil.check_image_exist(self.driver, template_path="testCase/ios/image/tpl1760938753682.png",resolution=(828, 1792),wait_time=2,expect=True, fail_msg="断言：image组件，mode为空，src传本地图片样式检测。期望显示WeChat图片")
        result_text = self.base.find_text(self.driver, "next", lang="en")
        self.base.tap_coordinates(result_text["target_coords"])
        # 3.scale to fill
